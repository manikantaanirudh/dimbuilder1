...
export function VaryingPropertiesPanel({
  ...
  const [draft, setDraft] = useState<VaryingPropertyDraft>(() =>
    emptyDraft(dimension.id),
  );
  ...
  const propertyDefinitions = useMemo(
    () =>
      getPropertyDefinitionsForDimension(
        ...
        draft.targetType as OneStreamPropertyTargetLevel,
      ),
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const [loadedValues, loadedMembers, loadedRelationships] =
          await Promise.all([
            fetchVaryingPropertyValues(projectId, {
        ...
        setStatus(
          `${loadedValues.length} varying ${loadedValues.length === 1 ? "value" : "values"}`,
        );
      } catch (caught) {
        if (!cancelled)
          setStatus(
            caught instanceof Error
              ? caught.message
              : "Failed to load varying properties",
          );
      }
    }
    void load();
  ...
  }, [dimension.id, projectId, refreshSignal]);
  ...
  useEffect(() => {
    ...
    if (
      propertyDefinitions.some(
        (definition) => definition.displayName === draft.propertyName,
      )
    )
      return;
    setDraft((current) => ({
      ...current,
      propertyName: propertyDefinitions[0]?.displayName ?? "",
    }));
    ...
    <Panel className="varying-properties-panel">
      ...
      <div className="varying-form">
        ...
        <div className="varying-actions">
          <ActionButton
            variant="primary"
            disabled={!draft.targetId || !draft.propertyName}
            onClick={() => void submit()}
          >
      ...
      <div
        className="varying-table"
        role="table"
        aria-label="Varying property values"
      >
        ...
        {values.map((value) => (
          <div key={value.id} className="varying-row" role="row">
            ...
            <span>
              <StatusBadge tone={value.isDefault ? "info" : "warning"}>
                {value.isDefault ? "Default" : "Override"}
              </StatusBadge>
            </span>
            <span className="row-actions">
              <IconButton
                aria-label={`Edit ${value.propertyName}`}
                title="Edit"
                onClick={() => edit(value)}
              >
                <Edit3 size={14} />
              ...
              <IconButton
                aria-label={`Delete ${value.propertyName}`}
                title="Delete"
        ...
        {values.length === 0 && (
          <div className="varying-empty">
            No varying properties configured for this dimension.
          </div>
        )}
      </div>
    </Panel>
...
function buildTargetOptions(
  ...
  if (targetType === "dimension")
    return [{ id: dimension.id, label: dimension.dimensionName }];
  if (targetType === "member")
    return members.map((member) => ({
      id: member.id,
      label: member.memberKey || member.id,
    }));
  return relationships.map((relationship) => ({
    id: relationship.id,
    label: `${relationship.parentKey || "(blank)"} -> ${relationship.childKey || "(blank)"}`,
  }));
}
...
function targetLabel(
  ...
  if (value.targetType === "member")
    return (
      members.find((member) => member.id === value.targetId)?.memberKey ||
      value.targetId
    );
  const relationship = relationships.find(
    (candidate) => candidate.id === value.targetId,
  );
  return relationship
...
function contextLabel(
  value: Pick<
    VaryingPropertyValueRecord,
    "cubeType" | "scenarioType" | "timeMember"
  >,
): string {
  const parts = [
