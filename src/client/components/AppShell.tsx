...
export function AppShell({
  ...
  const resolvedActiveDimensionId =
    activeWorkspace === PROJECT_OVERVIEW_VALUE
      ? null
  ...
  const activeDimension = useMemo(
    () =>
      store.dimensions.find(
        (dimension) => dimension.id === resolvedActiveDimensionId,
      ) ?? null,
  ...
  async function runValidation() {
    ...
    const result = await validateProject(
      store.selectedProjectId,
      appConfig.validation.duplicateMemberSeverity,
    );
        ...
        <header className="toolbar global-toolbar">
          ...
          >
            ...
            <span className="brand-wordmark">
              {appConfig.application.productName}
            </span>
          </button>
            ...
            <select
              ...
              value={
                showProjectOverview
                  ...
                  : (activeDimension?.id ?? PROJECT_OVERVIEW_VALUE)
              }
              onChange={(event) =>
                setActiveWorkspace(
                  event.currentTarget.value === PROJECT_OVERVIEW_VALUE
                    ? PROJECT_OVERVIEW_VALUE
          ...
          <ToolbarGroup className="toolbar-actions">
            ...
            {toolbar.showValidate && (
              <ActionButton
                title={
                  store.selectedProjectId
                    ? "Validate metadata"
                    : "Create or open a project before validating"
                }
            ...
            {toolbar.showSave && (
              <ActionButton
                ...
                title={
                  store.selectedProjectId
                    ? "Save project with a custom name"
                    : "No project open"
                }
            ...
            {toolbar.showUndoRedo && (
              <ActionButton disabled title="Undo" aria-label="Undo">
                <Undo2 size={16} />
              </ActionButton>
            )}
            {toolbar.showUndoRedo && (
              <ActionButton disabled title="Redo" aria-label="Redo">
                <RotateCcw size={16} />
              </ActionButton>
            )}
            <IconButton
        ...
        <main className="main" id="main-content">
          {configError && (
            <div className="banner error">
              Configuration failed to load. Using defaults: {configError}
            </div>
          )}
