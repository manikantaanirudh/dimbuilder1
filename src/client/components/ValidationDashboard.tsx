...
export function ValidationDashboard({
  ...
  const byRuleCode = useMemo(() => {
    const map = new Map<string, { code: string; severity: string; count: number; firstDimId: string }>();
    for (const issue of filteredIssues) {
      const existing = map.get(issue.code);
      if (existing) {
        existing.count++;
      } else {
        map.set(issue.code, { code: issue.code, severity: issue.severity, count: 1, firstDimId: issue.dimensionId });
      }
    }
      ...
      <div className="admin-page">
        ...
        {severityFilter && (
          <p style={{ fontSize: '0.82rem', color: 'var(--muted)', marginBottom: '0.5rem' }}>
            Showing <b>{filteredIssues.length}</b> {severityFilter} issue(s). <button className="chip" onClick={() => setSeverityFilter(null)}>Clear filter</button>
          </p>
        )}
        ...
        {byRuleCode.length > 0 && (
          <div className="validation-section">
            ...
            <table className="rules-table">
              <thead>
                <tr><th>Rule Code</th><th>Severity</th><th>Count</th><th></th></tr>
              </thead>
              <tbody>
                {byRuleCode.slice(0, 20).map(({ code, severity, count, firstDimId }) => (
                  <tr key={code} className="clickable-row" onClick={() => onNavigateDimension?.(firstDimId)}>
                    ...
                    <td><b>{count}</b></td>
                    <td style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Go to dimension</td>
                  </tr>
                ))}
              </tbody>
            </table>
