...
export function detectDuplicates(input: DuplicateDetectionInput): DuplicateGroup[] {
  ...
  for (let i = 0; i < members.length; i++) {
    if (processed.has(members[i].memberKey)) continue;
    const group: string[] = [members[i].memberKey];
    let bestSimilarity = 0;
    let bestMethod = 'levenshtein';

    for (let j = i + 1; j < members.length; j++) {
      if (processed.has(members[j].memberKey)) continue;

      let maxSim = 0;
      let method = 'levenshtein';

      if (config.methods.includes('levenshtein')) {
        const sim = levenshteinSimilarity(members[i].memberKey, members[j].memberKey);
        if (sim > maxSim) { maxSim = sim; method = 'levenshtein'; }
      }

      if (config.methods.includes('soundex')) {
        const sim = soundexSimilarity(members[i].memberKey, members[j].memberKey);
        if (sim > maxSim) { maxSim = sim; method = 'soundex'; }
      }

      if (config.methods.includes('prefix')) {
        const sim = prefixSimilarity(members[i].memberKey, members[j].memberKey);
        if (sim > maxSim) { maxSim = sim; method = 'prefix'; }
      }

      if (maxSim >= threshold) {
        group.push(members[j].memberKey);
        processed.add(members[j].memberKey);
    ...
    if (group.length > 1) {
      processed.add(members[i].memberKey);
      groups.push({ members: group, similarity: bestSimilarity, method: bestMethod });
    }
  }
