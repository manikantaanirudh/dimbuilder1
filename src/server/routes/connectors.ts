...
export function createConnectorRouter(repos: Repositories, _config: AppConfig): Router {
  ...
  router.get("/", async (_req, res) => {
    const connectors = (await repos.connectors.list()).map(c => ({
      ...c,
  ...
  router.post("/", async (req, res) => {
    ...
    const connector = await repos.connectors.create({
      ...parsed.data,
  ...
  router.patch("/:id", async (req, res) => {
    ...
    const updated = await repos.connectors.update(req.params.id, parsed.data);
  ...
  router.delete("/:id", async (req, res) => {
    const existing = await repos.connectors.getById(req.params.id);
    if (!existing) return res.status(404).json({ error: "Connector not found" });
    await repos.connectors.delete(req.params.id);
    res.json({ ok: true });
  });

  router.post("/:id/test", async (req, res) => {
    const connector = await repos.connectors.getById(req.params.id);
    ...
    try {
      ...
      await repos.connectors.setLastTested(connector.id);
      res.json(result);
    } catch (err) {
  ...
  router.post("/:id/preview", async (req, res) => {
    ...
    const rule = await repos.mappingRules.getById(mappingRuleId);
  ...
  router.get("/:id/mappings", async (req, res) => {
    const connector = await repos.connectors.getById(req.params.id);
    if (!connector) return res.status(404).json({ error: "Connector not found" });
    res.json(await repos.mappingRules.listByConnector(req.params.id));
  });
  ...
  router.post("/:id/mappings", async (req, res) => {
    ...
    const rule = await repos.mappingRules.create({
...
export function createMappingRouter(repos: Repositories): Router {
  ...
  router.patch("/:id", async (req, res) => {
    ...
    const updated = await repos.mappingRules.update(req.params.id, parsed.data);
    if (!updated) return res.status(404).json({ error: "Mapping rule not found" });
    res.json(updated);
  });

  router.delete("/:id", async (req, res) => {
    const existing = await repos.mappingRules.getById(req.params.id);
    if (!existing) return res.status(404).json({ error: "Mapping rule not found" });
    await repos.mappingRules.delete(req.params.id);
    res.json({ ok: true });
  });
...
export function createSyncJobRouter(repos: Repositories): Router {
  ...
  router.get("/", async (req, res) => {
    ...
    res.json(await repos.syncJobs.list({ connectorId, projectId }));
  });
  ...
  router.post("/", async (req, res) => {
    ...
    const connector = await repos.connectors.getById(parsed.data.connectorId);
    if (!connector) return res.status(400).json({ error: "Connector not found" });
    const rule = await repos.mappingRules.getById(parsed.data.mappingRuleId);
    ...
    const job = await repos.syncJobs.create({
      ...parsed.data,
      createdBy: req.user?.id ?? "system"
    });
  ...
  router.post("/:id/run", async (req, res) => {
    const job = await repos.syncJobs.getById(req.params.id);
    if (!job) return res.status(404).json({ error: "Sync job not found" });

    const connector = await repos.connectors.getById(job.connectorId);
    if (!connector) return res.status(500).json({ error: "Connector no longer exists" });

    const rule = await repos.mappingRules.getById(job.mappingRuleId);
    if (!rule) return res.status(500).json({ error: "Mapping rule no longer exists" });

    const run = await repos.syncRuns.create({ jobId: job.id });
    ...
    try {
      ...
      for (const member of pipelineResult.members) {
        if (member.memberKey) {
          await repos.memberSourceRegistry.upsert({
      ...
      await repos.syncRuns.complete(run.id, {
        status,
      ...
      await repos.syncJobs.updateLastRun(job.id);
      const completedRun = await repos.syncRuns.getById(run.id);
      ...
      await repos.syncJobs.updateLastRun(job.id);
      const failedRun = await repos.syncRuns.getById(run.id);
      res.json(failedRun);
    }
  });
  ...
  router.get("/:id/runs", async (req, res) => {
    const job = await repos.syncJobs.getById(req.params.id);
    if (!job) return res.status(404).json({ error: "Sync job not found" });
    res.json(await repos.syncRuns.listByJob(req.params.id));
  });
...
export function createSyncRunRouter(repos: Repositories): Router {
  ...
  router.get("/:id", async (req, res) => {
    const run = await repos.syncRuns.getById(req.params.id);
...
export function createSourceRegistryRouter(repos: Repositories): Router {
  ...
  router.get("/:projectId/source-registry", async (req, res) => {
    const { dimensionType } = req.query as { dimensionType?: string };
    res.json(await repos.memberSourceRegistry.listByProject(req.params.projectId, dimensionType));
  });
