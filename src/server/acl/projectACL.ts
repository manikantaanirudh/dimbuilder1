...
export function requireProjectRole(repos: Repositories, minimumRole: ProjectRole) {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    const members = repos.projectMembers.listByProject(projectId);

    // If no ACL entries exist, allow all (backwards compatible)
    if (members.length === 0) return next();

    // Check user's role
    const userRole = repos.projectMembers.getUserRole(projectId, userId) as ProjectRole | null;
    if (!userRole) {
...
export function createProjectACLRouter(repos: Repositories): Router {
  ...
  router.get("/projects/:id/members", (req, res) => {
    ...
    const members = repos.projectMembers.listByProject(project.id);
    res.json(members);
  });
  ...
  router.post("/projects/:id/members", (req, res) => {
    ...
    const currentMembers = repos.projectMembers.listByProject(project.id);
    if (currentMembers.length > 0) {
      const requesterRole = repos.projectMembers.getUserRole(project.id, req.user?.id ?? "system");
      if (requesterRole !== 'owner') {
        return res.status(403).json({ error: "Only project owners can manage membership" });
      }
    }

    const member = repos.projectMembers.add({
  ...
  router.delete("/projects/:id/members/:userId", (req, res) => {
    ...
    repos.projectMembers.remove(project.id, req.params.userId);
    res.status(204).end();
  });

  // GET /users/:userId/projects — list all projects a user has access to
  router.get("/users/:userId/projects", (req, res) => {
    const memberships = repos.projectMembers.listByUser(req.params.userId);
